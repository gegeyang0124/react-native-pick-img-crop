...
public class PictureImageGridAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if (getItemViewType(position) == PictureConfig.TYPE_CAMERA) {
            ...
            selectImage(contentHolder, isSelected(image),selectedPosition(image), false);
    ...
    public int selectedPosition(LocalMedia image) {
    ...
    private void changeCheckboxState(ViewHolder contentHolder, LocalMedia image) {
        ...
        selectImage(contentHolder, !isChecked,selectedPosition(image), true);
        notifyDataSetChanged();
        if (imageSelectChangedListener != null) {
            imageSelectChangedListener.onChange(selectImages);
        }
     ...
     * @param isAnim
     */
    public void selectImage(ViewHolder holder, boolean isChecked,int selectedPosition, boolean isAnim) {
        holder.check.setSelected(isChecked);

        if (isChecked) {
            holder.check.setText(""+selectedPosition);
            if (isAnim) {
                if (animation != null) {
